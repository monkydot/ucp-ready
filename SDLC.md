...
- **Designer** — owns the flow and its states before the code exists, and the design contract the UI review reads back. May be a human, `om-ux-shape` for the shaping, `om-ux-review-pr` for the pass over a PR's screens. This repo ships a CLI/library with no UI, so this role rarely engages.
...
Since this engine parses and reports on payment-related protocol data (UCP payment handlers, AP2 mandates) without ever touching money or credentials itself, most changes here read as `risk-medium` at most; treat anything that adds network calls to third-party endpoints or handles signing-key material as `risk-high`.
...
- Apply `needs-qa` to UI changes, new features, and other user-facing behavior that needs manual exercise. This repo has no UI; `needs-qa` is expected to be rare here (e.g. a change to interactive CLI prompts).
- `skip-qa` is the explicit opt-out for docs-only, dependency-only, CI-only, test-only, and similarly low-risk non-user-facing changes — the default for most PRs in this CLI/library repo. Never combine it with `needs-qa`.
